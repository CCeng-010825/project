...
const getSkuData = async (pager = 1) => {
  pageNo.value = pager
  let result: ReqSkuInfoModel = await reqSkuList(pageNo.value, pageSize.value)
  tableData.value = result.data.records
  total.value = result.data.total
}
// console.log(result)
const upDateSale = async (row: Records) => {
  if (row.isSale === 1) {
    //下架操作
...
const findSku = (row: Records) => {
  drawer.value = true
  reqSkuInfo(row.id).then((res: ReqSkuInfoData) => {
